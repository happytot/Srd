...
const StatCard = ({ title, value, subtext, icon }) => (
  <div className="stat-card">
    ...
    <p className="text-xs text-zinc-500 mt-1 flex gap-1">
      <span className={subtext.includes('+') ? "text-emerald-600 font-bold" : ""}>
        {subtext.split(' ')[0]}
      </span>
      <span className="text-zinc-400 inline-block">
...
const Overview = ({ globalDateRange, globalCustomStart, globalCustomEnd }) => {
  ...
  const trendDays = useMemo(() => {
    ...
    if (globalDateRange === 'Monthly' || globalDateRange === 'Month to Date') return now.getDate();
  ...
  useEffect(() => {
    ...
    const unsubInventory = onSnapshot(inventoryQuery, (snapshot) => {
      ...
      setStats(prev => ({ ...prev, lowStock: lowStockList.slice(0, 4) }));
      setLoading(false);
    });
      ...
      if (date >= startCurrent && date <= endCurrent) {
        ...
        if (order.items) {
          order.items.forEach(item => {
            if (!productCounts[item.name]) productCounts[item.name] = { sales: 0, revenue: 0 };
  ...
  const revenueVariance = stats.previousRevenue === 0
    ? (stats.currentRevenue > 0 ? 100 : 0)
    : ((stats.currentRevenue - stats.previousRevenue) / stats.previousRevenue) * 100;

  const revenueDiffText = stats.previousRevenue === 0
    ? "No prior data"
    : `${revenueVariance > 0 ? '+' : ''}${revenueVariance.toFixed(1)}% vs prior`;

  return (
    <>
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
        <StatCard
          title={`${globalDateRange === 'Today' ? "Today's" : "Period"} Revenue`}
          value={`₱${stats.currentRevenue.toLocaleString(undefined, { minimumFractionDigits: 2 })}`}
          subtext={revenueDiffText}
          icon=""
        />
        ...
        <div className="lg:col-span-2 content-card h-[350px]">
          ...
          <div className="w-full h-40 mt-6 border-b border-zinc-100 relative">
            <svg viewBox="0 -50 400 150" className="w-full h-full overflow-visible" onMouseLeave={() => setHoveredPoint(null)}>
              <path d={trendDataObj.path} fill="none" stroke="black" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" />
              {trendDataObj.points.map((pt, i) => (
                <g key={i} className="cursor-pointer" onMouseEnter={() => setHoveredPoint(i)}>
                  <circle cx={pt.x} cy={pt.y} r="16" fill="transparent" />
                  <circle cx={pt.x} cy={pt.y} r={hoveredPoint === i ? 4.5 : 0} fill={hoveredPoint === i ? "black" : "transparent"} stroke={hoveredPoint === i ? "white" : "transparent"} strokeWidth="1.5" />
                  {hoveredPoint === i && stats.salesTrend[i] && (
                    <g>
                      ...
                      <text x={pt.x} y={pt.y - 32} fill="white" fontSize="13" fontWeight="bold" textAnchor="middle">₱{stats.salesTrend[i].sales.toLocaleString()}</text>
          ...
          <div className="flex justify-between items-center text-[10px] text-zinc-400 mt-3 font-medium px-1">
            <span>{stats.salesTrend[0]?.dateLabel}</span>
            <span>{stats.salesTrend[Math.floor(stats.salesTrend.length / 2)]?.dateLabel}</span>
            <span>{stats.salesTrend[stats.salesTrend.length - 1]?.dateLabel}</span>
          </div>
        </div>
            ...
            <tbody className="divide-y divide-zinc-50">
              {stats.bestSelling.map((p) => (
                <tr key={p.name} className="table-row">
                  <td className="table-data-cell font-bold truncate">{p.name}</td>
  ...
  if (isAuthLoading) {
    return (
      <div className="min-h-screen bg-zinc-50 flex flex-col justify-center items-center font-sans">
        <div className="w-12 h-12 bg-black rounded-xl flex items-center justify-center text-white font-bold text-xl shadow-lg animate-pulse">C</div>
        <p className="mt-4 text-sm text-zinc-500 font-medium">Loading Dashboard...</p>
      </div>
    );
  }
          ...
          <div className="flex flex-wrap items-center gap-4 w-full xl:w-auto">
            {['Overview', 'Sales Analytics', 'Sales Reports'].includes(activeTab) && (
              <GlobalDateFilter
                ...
                setGlobalCustomStart={setGlobalCustomStart}
                globalCustomEnd={globalCustomEnd}
                setGlobalCustomEnd={setGlobalCustomEnd}
              />
            )}
